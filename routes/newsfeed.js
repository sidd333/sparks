...
router.post('/uploadprofile', upload.single('img'), (req, res, next) => {
    ...
    if (req.file && req.file.filename) obj.img = {
        data: fs.readFileSync(path.join(__dirname, '..', 'uploads', req.file.filename)),
        contentType: 'image/png'
    };
...
router.post('/newsfeed/follow/:_id', async (req, res) => {
    ...
    try {
        ...
        if (personToBeFollowed.followers.includes(req.user._id)) {
            const apply = await personToBeFollowed.updateOne({ $pull: { followers: req.user._id } });
            const follow = await currentUser.updateOne({ $pull: { following: req.params._id } });
            // TODO : ONE MORE UPDATE FUCTION ,CREATE ANOTHER KEY FOR FOLLOWERS AND PUT ITS UPDATE FUNCTION HERE
        } else {
            const follow = await currentUser.updateOne({ $push: { following: req.params._id } });
            const apply = await personToBeFollowed.updateOne({ $push: { followers: req.user._id } });
...
router.post('/newsfeed/search', async (req, res) => {
    ...
    try {
        ...
        const secUsers = await user.find({ "username": new RegExp( '.*'+nameSim+'.*','i' )});
    ...
    try {
        ...
        if(await req.isAuthenticated()) {//find the post to update Likes
            ...
            if (post.likedby.includes(req.user._id)) {
                ...
                const updateL = await imgModel.updateOne({ "_id": req.params._id }, { $pull: { likedby: req.user._id } });
            } else {
                const updateLikes = await post.updateOne({ likes: post.likes + 1 });
